...
You can see a `Part` helper wraps the main transformation. It will save
all other node keys that were not used in previous transformations.
...
Both tree shapes are provided as an argument to `Map` that will
bind transformations together creating a mapping:
...
* `Obj map[string]Op` - verifies that current node is an object and checks
