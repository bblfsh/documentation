...
Make sure that you've correctly set your GOROOT and GOPATH environment
variables. 

### Installing

Install the Babelfish SDK. Native drivers and the external tools can be developed
on any programming language but the non-native driver is developed using Go so you
...
### Creating the driver's filesystem tree
...
If the update gives you some problem you can try to delete the
...
- tests: source code, native AST, and UAST files that with for 
...
When the driver executes the parser, the former entry point should Officially a
...
(uast/code-to-ast.html) page, for example for including comments and whitespace
...
protocol](driver/internal-protocol.md)
page.
...
AST"](uast/code-to-ast.html) page.
...
are explained in the [annotations section](driver/annotations.md).
